  ...
  return (
    <>
      ...
      <AnimatePresence>
        {confirmPrompt && (
          <motion.div
            className="fixed inset-0 z-[90] flex items-center justify-center bg-stone-950/35 px-4 py-6 backdrop-blur-[2px]"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          >
            <motion.div
              className="w-full max-w-sm overflow-hidden rounded-[24px] border border-rose-100 bg-[#fffaf5] shadow-[0_28px_80px_rgba(92,38,38,0.28)]"
              ...
              <div className="border-b border-rose-100 bg-gradient-to-r from-rose-50 via-orange-50 to-amber-50 px-5 py-4">
                <div className="flex items-center gap-3">
                  <div className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-white text-rose-500 shadow-sm">
                    <span aria-hidden="true">{confirmPrompt.tone === 'danger' ? '🗑️' : '🌱'}</span>
                  </div>
