...
test('World renders continuous PBR terrain separately from authoritative hex picking', async () => {
  ...
  assert.match(world, /<HexPBRTerrain[^>]*seed=\{snapshot\.world\.seed\}/);
...
test('floating island side is one boundary-driven PBR geological shell', async () => {
  const world = await source('components/hex-world/HexWorld3D.tsx');
  const shell = await source('components/hex-world/pbr/HexPBRCliff.tsx');
  const boundary = await source('lib/hex-world/island-boundary.ts');
  assert.match(world, /HexPBRCliff/);
  assert.match(world, /<HexPBRCliff[^>]*seed=\{snapshot\.world\.seed\}/);
  assert.doesNotMatch(world, /<HexIslandUnderside\b|<HexIslandCliffShell\b/);
