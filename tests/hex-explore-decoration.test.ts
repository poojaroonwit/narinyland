...
test('explore decoration module exists and is deterministic for a world seed', async () => {
  const decoration = await loadExploreDecoration();
  assert.ok(decoration, 'explore decoration module must exist');
  ...
  const first = decoration.getExploreDecorationSamples({ seed: 'cozy-seed', tiles, profile });
  const second = decoration.getExploreDecorationSamples({ seed: 'cozy-seed', tiles, profile });
...
test('explore decoration skips locked land and stays inside quality budgets', async () => {
  ...
  const samples = decoration.getExploreDecorationSamples({ seed: 'bounded', tiles: [unlocked, locked], profile });
...
test('mobile explore density is lower than medium and high', async () => {
  ...
  const count = (profile: typeof high) => decoration.getExploreDecorationSamples({ seed: 'density', tiles, profile }).length;
