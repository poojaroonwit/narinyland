...
const GAMEPLAY_LOOP_MS = 11000;

const GAMEPLAY_STAGES = [
  {
    label: 'Arrive',
    icon: 'fa-house-chimney',
    eyebrow: 'A world wakes up',
    copy: 'Start with one tiny floating home that already feels like yours.',
  },
  {
    label: 'Build',
    icon: 'fa-hammer',
    eyebrow: 'Shape your homestead',
    copy: 'Place useful little buildings and turn open land into a place with purpose.',
  },
  {
    label: 'Grow',
    icon: 'fa-seedling',
    eyebrow: 'Make it feel alive',
    copy: 'Grow the garden, add flowers and trees, and make every corner more personal.',
  },
  {
    label: 'Expand',
    icon: 'fa-up-right-and-down-left-from-center',
    eyebrow: 'Unlock more land',
    copy: 'Open new hexes as your shared world grows and make room for what comes next.',
  },
  {
    label: 'Together',
    icon: 'fa-heart',
    eyebrow: 'One world for two',
    copy: 'Come back together, keep building, and let the homestead become your shared story.',
  },
] as const;
      ...
      <section className="relative z-10 h-full w-full">
        ...
        >
          <motion.div
            ...
            transition={prefersReducedMotion ? { duration: 0 } : { duration: 14, repeat: Infinity, ease: 'easeInOut' }}
          >
            <HexWorld3D snapshot={activeSnapshot} graphicsQuality="medium" />
          </motion.div>
        </motion.div>
        ...
        <AnimatePresence initial={false}>
          <motion.div
            ...
            animate={{ opacity: 0 }}
            exit={{ opacity: 0 }}
            transition={{ duration: prefersReducedMotion ? 0 : 0.48 }}
          />
        </AnimatePresence>
        ...
        <div className="pointer-events-none absolute inset-0 z-[60]">
          ...
          >
            <div className="hidden h-9 items-center gap-2 rounded-full bg-white/28 px-3.5 text-stone-700 shadow-sm backdrop-blur-xl sm:flex">
              <motion.span
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.97 }}
              className="flex h-10 items-center gap-2 rounded-full bg-white/[0.78] px-4 text-xs font-black text-stone-800 shadow-lg backdrop-blur-xl disabled:cursor-wait disabled:opacity-70"
            >
          ...
          <div className="absolute left-4 right-4 top-[5.25rem] flex justify-end md:right-6 md:top-[5.6rem]">
            <div className="pointer-events-auto w-full max-w-[20rem] rounded-[1.5rem] bg-white/[0.34] p-3.5 shadow-xl shadow-emerald-950/[0.05] backdrop-blur-xl md:p-4">
              <div className="flex items-center justify-between gap-3">
                <span className="text-[9px] font-black uppercase tracking-[0.18em] text-emerald-800">Gameplay loop</span>
                <span className="text-[9px] font-bold text-stone-500">11 sec</span>
              </div>

              <div className="mt-3 flex gap-1.5" aria-label={`Gameplay stage: ${activeStage.label}`}>
              ...
              <AnimatePresence mode="wait" initial={false}>
                <motion.div
                  ...
                  initial={prefersReducedMotion ? { opacity: 1 } : { opacity: 0, y: 7 }}
                  animate={{ opacity: 1, y: 0 }}
                  exit={prefersReducedMotion ? { opacity: 1 } : { opacity: 0, y: -5 }}
                  transition={{ duration: prefersReducedMotion ? 0 : 0.28 }}
                  className="mt-3 flex items-start gap-3"
                >
                  <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-emerald-100/80 text-emerald-700 shadow-sm">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="mt-0.5 text-[11px] font-semibold leading-relaxed text-stone-600">{activeStage.copy}</p>
                  </div>
                </motion.div>
              </AnimatePresence>
            </div>
          ...
          >
            <div className="max-w-[36rem]">
              <div className="mb-2.5 inline-flex items-center gap-2 rounded-full bg-white/[0.38] px-3 py-1.5 text-[9px] font-black uppercase tracking-[0.17em] text-emerald-800 shadow-sm backdrop-blur-xl">
              ...
              <p className="mt-2.5 max-w-lg text-xs font-semibold leading-relaxed text-stone-700 drop-shadow-[0_1px_10px_rgba(255,255,255,0.95)] sm:text-sm md:text-base">
                Build, grow, expand, and come home to the same living homestead—together.
              </p>
              <div className="mt-4 flex flex-wrap items-center gap-2.5">
                ...
                <Link
                  href="/signup"
                  className="pointer-events-auto flex min-h-[46px] items-center gap-2 rounded-full bg-white/48 px-4 text-xs font-black text-stone-700 shadow-md backdrop-blur-xl transition hover:bg-white/68"
                >
          ...
          <div className="absolute bottom-[calc(1.5rem+env(safe-area-inset-bottom))] right-5 hidden md:block">
            <div className="rounded-full bg-white/28 px-3 py-1.5 text-[9px] font-black uppercase tracking-[0.16em] text-stone-500 shadow-sm backdrop-blur-lg">
              {activeStageIndex + 1} / {GAMEPLAY_STAGES.length} · autoplay
            </div>
          </div>
        </div>
      </section>
