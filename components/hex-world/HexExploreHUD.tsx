...
export function HexExploreHUD({
  ...
  return (
    <div className="pointer-events-none fixed inset-x-3 bottom-[calc(5.7rem+env(safe-area-inset-bottom))] z-[92] flex items-end justify-between gap-2 md:inset-x-5">
      <div className="pointer-events-auto min-w-0 max-w-[46vw] rounded-[1.15rem] border border-white/55 bg-stone-900/78 p-2.5 text-white shadow-2xl shadow-stone-950/20 backdrop-blur-xl sm:w-[260px] sm:max-w-none">
      ...
      <div className="pointer-events-none fixed bottom-[calc(10.6rem+env(safe-area-inset-bottom))] left-1/2 max-w-[calc(100vw-2rem)] -translate-x-1/2 rounded-full bg-stone-900/68 px-3 py-1 text-center text-[9px] font-bold text-white/82 backdrop-blur-md sm:hidden">Drag to look · movement requires a keyboard</div>
    </div>
  );
}
