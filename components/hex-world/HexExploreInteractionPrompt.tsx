...
function fallbackName(target: HexExploreInteractionTarget): string {
  switch (target.role) {
...
export function HexExploreInteractionPrompt({
  target,
  disabled = false,
  onInteract,
}: {
  ...
  const name = getBuildingDefinition(target.building.buildingKey)?.name ?? fallbackName(target);
  ...
  return (
    <>
      <div className="pointer-events-none fixed bottom-[calc(11.1rem+env(safe-area-inset-bottom))] left-1/2 z-[94] hidden -translate-x-1/2 items-center gap-2 rounded-full border border-white/45 bg-stone-900/74 px-3 py-2 text-white shadow-xl backdrop-blur-xl sm:flex">
        ...
        <span className="text-[10px] font-black">Interact</span>
      ...
      <button
        ...
        className="pointer-events-auto fixed bottom-[calc(10.7rem+env(safe-area-inset-bottom))] right-[calc(0.85rem+env(safe-area-inset-right))] z-[94] min-h-[44px] min-w-[96px] rounded-2xl border border-white/65 bg-stone-900/90 px-3 py-2 text-left text-white shadow-2xl backdrop-blur-xl sm:hidden"
        aria-label={`Interact with ${name}`}
      >
        <span className="block text-[10px] font-black">Interact</span>
        <span className="block max-w-[130px] truncate text-[8px] font-bold text-white/65">{name}</span>
      </button>
    </>
  );
}
