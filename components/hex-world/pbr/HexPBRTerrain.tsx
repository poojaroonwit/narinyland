...
function usePBRMaps(material: HexPBRMaterialName, profile: HexQualityProfile, repeat: [number, number]) {
...
export function HexPBRTerrain({ tiles, seed, profile }: { tiles: HexTileDTO[]; seed: string; profile: HexQualityProfile }) {
  const grass = usePBRMaps('grass', profile, [2.7, 2.7]);
  const soil = usePBRMaps('soil', profile, [2.45, 2.45]);
  const path = usePBRMaps('path', profile, [2.15, 2.15]);
  const stone = usePBRMaps('cliff', profile, [2.35, 2.35]);
