...
export function HexWorldToolbar({
  ...
  const primary = 'min-h-[42px] min-w-[50px] rounded-xl px-2 text-[9px] font-black transition active:scale-95 sm:min-w-[58px] sm:px-2.5 sm:text-[10px]';
  ...
  const modeClass = (mode: HexViewMode) => `h-8 rounded-lg px-2.5 text-[9px] font-black transition sm:text-[10px] ${viewMode === mode ? 'bg-stone-900 text-white shadow-md' : 'text-stone-500 hover:bg-white/75'}`;
    ...
    <div className="pointer-events-none fixed bottom-[calc(5.7rem+env(safe-area-inset-bottom))] left-1/2 z-[90] flex max-w-[calc(100vw-1rem)] -translate-x-1/2 flex-col items-center gap-1.5">
      ...
      <div className="pointer-events-auto flex max-w-full items-center gap-1 rounded-[1.15rem] border border-white/75 bg-[#fffdf7]/90 p-1.5 shadow-2xl shadow-emerald-950/[0.08] backdrop-blur-xl sm:gap-1.5">
        ...
        <div className="ml-0.5 flex flex-col gap-1 border-l border-stone-900/[0.07] pl-1 sm:pl-1.5">
          {viewMode === 'world' && <button type="button" onClick={onExpand} aria-label="Grow land" title="Grow land" className="h-[19px] w-8 rounded-md bg-amber-50 text-[10px] font-black text-amber-700">⬡+</button>}
          <button type="button" onClick={onResetView} aria-label={viewMode === 'person' ? 'Reset player view' : 'Reset world view'} title="Reset view" className="h-[19px] w-8 rounded-md bg-stone-100 text-[10px] font-black text-stone-500">⌂</button>
        </div>
      </div>
      {viewMode === 'person' && (
